...
// CREAR empleado con fetch API (async/await)
btnCrear = document.querySelector("#BtnCrearUsuario");
btnCrear.addEventListener("click", () => {
  Swal.fire({
    title: '<span class="text-success fw-bold">Crear usuario</span>',
    html: `
    ...
    preConfirm: async () => {
      try {
        ...
        const response = await fetch("../../controllers/agregar_usuario.php", {
          method: "POST",
          body: formData,
        });
    ...
    if (resultado.isConfirmed && resultado.value?.success) {
      Swal.fire("Éxito", resultado.value.message, "success").then(() => {
        location.reload();
      });
    }
  });
